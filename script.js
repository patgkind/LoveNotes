...
function createOrUpdateNoteDom(note,focus=false,adding=false){
  ...
  del.addEventListener('click', async ev=>{
    ev.stopPropagation(); if(mode!=='delete') return;
  ...
  // pin badge
  const pin=document.createElement('div'); pin.className='pin-badge'; pin.innerText='📌'; pin.title=note.pinned?'Unpin':'Pin';
  ...
  // color input (always part of DOM)
  const colorInput=document.createElement('input');
  colorInput.type='color'; colorInput.className='color-input';
  colorInput.value=note.color||'#fffa91'; el.appendChild(colorInput);

  // color button
  const colorBtn=document.createElement('button'); colorBtn.className='color-btn';
  colorBtn.style.background=note.color||'#fff'; colorBtn.title='Change color';
  colorBtn.addEventListener('click', ev=>{ ev.stopPropagation(); colorInput.click(); });
  el.appendChild(colorBtn);

  colorInput.addEventListener('input', async ev=>{
    const v=ev.target.value;
    el.style.background=v; colorBtn.style.background=v;
    await editNoteBackend({id:note.id, color:v});
    note.color=v; noteHashes[note.id]=hashNote(note);
  });
  ...
  if(note.image){
    ...
    img.style.maxWidth='100%'; img.style.maxHeight='40%';
    img.style.display='block'; img.style.marginBottom='6px'; img.style.borderRadius='6px'; img.style.zIndex=1;
    el.appendChild(img);
  }

  // textarea
  const ta=document.createElement('textarea');
  ta.value=note.text||''; ta.disabled=!(adding||(mode==='edit' && taIsSelected(note.id)));
  ta.style.color=textColor(note.color||'#000'); el.appendChild(ta);

  el.addEventListener('click', ev=>{
    ev.stopPropagation();
    if(mode==='edit'){
      Object.values(noteMap).forEach(x=>{ x.classList.remove('editing'); const xt=x.querySelector('textarea'); if(xt) xt.disabled=true; });
      el.classList.add('editing'); ta.disabled=false; ta.focus(); selectedForEdit=note.id;
    }
  });
...
function attachDrag(el,id){
  ...
  el.addEventListener('mousedown', ev=>{
    const tag=ev.target.tagName; if(["TEXTAREA","INPUT","BUTTON","IMG","svg"].includes(tag)) return;
...
// optional: poll only when not editing
