...
function createOrUpdateNoteDom(note,focus=false,adding=false){
  ...
  ta.addEventListener('blur', async()=>{
    ...
    if(newText!==note.text) await editNoteBackend({id:note.id,text:newText});
    el.classList.remove('adding'); el.classList.remove('editing'); selectedForEdit=null;
    mode='normal'; board.classList.remove('edit-outline'); // exit edit mode after done editing
  });
...
async function loadNotes(realTime=true){
  try{
